...
app.post('/compile', (req, res) => {
  try {
    ...
    if (sourceCode) {
      // Zastąp pragma solidity 0.7.5 na ^0.7.5 aby zadziałało z kompilerem 0.7.6
      const modifiedSourceCode = sourceCode.replace(/pragma solidity 0.7.5;/g, 'pragma solidity ^0.7.5;');
      
      // Wyciągnij nazwę kontraktu z kodu źródłowego
      const contractNameMatch = modifiedSourceCode.match(/contract\s+(\w+)/);
      const contractName = contractNameMatch ? contractNameMatch[1] : 'CustomContract';
      
      const compiledData = compileContract(modifiedSourceCode, contractName);
